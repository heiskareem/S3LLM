...
subroutine read_vars(in_file,bounds,mode,nsets)
     ...
     if (errcode .ne. 0) stop
     call fio_read(18,'grc_pp%elevation', grc_pp%elevation(begg:endg), errcode=errcode)
     if (errcode .ne. 0) stop
     call fio_read(18,'grc_pp%froudenum', grc_pp%froudenum(begg:endg), errcode=errcode)
     if (errcode .ne. 0) stop
     ...
     if (errcode .ne. 0) stop
     call fio_read(18,'col_pp%is_fates', col_pp%is_fates(begc:endc), errcode=errcode)
     if (errcode .ne. 0) stop
     ...
     if (errcode .ne. 0) stop
     call fio_read(18,'veg_pp%sp_pftorder_index', veg_pp%sp_pftorder_index(begp:endp), errcode=errcode)
     if (errcode .ne. 0) stop
     ...
     if (errcode .ne. 0) stop
     call fio_read(18,'col_ws%frac_iceold', col_ws%frac_iceold(begc:endc,:), errcode=errcode)
     if (errcode .ne. 0) stop
     ...
     call fio_read(18,'ch4_vars%grnd_ch4_cond_col', ch4_vars%grnd_ch4_cond_col(begc:endc), errcode=errcode)
     if (errcode .ne. 0) stop
     ...
     if (errcode .ne. 0) stop
     call fio_read(18,'lakestate_vars%betaprime_col', lakestate_vars%betaprime_col(begc:endc), errcode=errcode)
     if (errcode .ne. 0) stop
     call fio_read(18,'lakestate_vars%savedtke1_col', lakestate_vars%savedtke1_col(begc:endc), errcode=errcode)
     if (errcode .ne. 0) stop
     ...
     if (errcode .ne. 0) stop
     call fio_read(18,'lakestate_vars%lakeresist_col', lakestate_vars%lakeresist_col(begc:endc), errcode=errcode)
     if (errcode .ne. 0) stop
     end if 
     call fio_close(18) 
